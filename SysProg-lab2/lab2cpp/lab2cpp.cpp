...
UINT __cdecl PolThread(LPVOID lpParameter)
{
    int id = int(lpParameter);
    auto StopName = "EventStop" + toStr(id);
    auto MessageName = "NewMessage" + toStr(id);
    HANDLE hMutex = CreateMutex(NULL, FALSE, "PolMutex");
...
void start() {
    ...
    auto hMutex = CreateMutex(NULL, FALSE, "PolMutex");
    ...
    while (true) {
        ...
        case 0:
        {
            ...
            HANDLE hThread = AfxBeginThread(PolThread, (LPVOID)thread_index++);
            SetEvent(evSubmit);
            break;
        }
        case 1:
